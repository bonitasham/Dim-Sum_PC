...
public class LogicScript : MonoBehaviour
{
    ...
    public List<string> order6 = new List<string>();
    // etc...
    ...
    public GameObject choiceTopLeft;
    ...
    private void Start()
    {
        ...
        order1.InsertRange(order1.Count, new string[] { "Siu mai", "Chicken feet", "Shrimp dumpling", "Sponge cake", "Cha siu bun", "Meatball" });
        order2.InsertRange(order2.Count, new string[] { "Meatball", "Siu mai", "Chicken feet", "Shrimp dumpling", "Sponge cake", "Cha siu bun" });
        order3.InsertRange(order3.Count, new string[] { "Cha siu bun", "Meatball", "Siu mai", "Chicken feet", "Shrimp dumpling", "Sponge cake" });
        order4.InsertRange(order4.Count, new string[] { "Sponge cake", "Cha siu bun", "Meatball", "Siu mai", "Chicken feet", "Shrimp dumpling" });
        order5.InsertRange(order5.Count, new string[] { "Shrimp dumpling", "Sponge cake", "Cha siu bun", "Meatball", "Siu mai", "Chicken feet" });
        order6.InsertRange(order6.Count, new string[] { "Chicken feet", "Shrimp dumpling", "Sponge cake", "Cha siu bun", "Meatball", "Siu mai" });


        // add all the orders to the orders list
        orders.InsertRange(orders.Count, new List<string>[] { order1, order2, order3, order4, order5, order6 });
    }
    ...
    public void Hint2()
    {
        // Check if exactly 3 choices are inactive to determine if the hint has been used
        bool hintUsed = CountInactiveChoices() == 3;

        if (hintUsed)
        {
            // The hint has already been used, do nothing.
            return;
        }

        List<Transform> incorrectChoices = new List<Transform>();

        // Populate the list of incorrect choices and shuffle it
        for (int i = 0; i < choiceLocations.Length; i++)
        {
            if (choiceLocations[i].transform.GetChild(0).name != correctAnswer)
            {
                incorrectChoices.Add(choiceLocations[i].transform.GetChild(0));
            }
        }

        ShuffleList(incorrectChoices);

        // Disable the first 3 choices in the shuffled list
        for (int i = 0; i < Mathf.Min(3, incorrectChoices.Count); i++)
        {
            incorrectChoices[i].gameObject.SetActive(false);
        }
    }

    private int CountInactiveChoices()
    {
        int inactiveCount = 0;
        for (int i = 0; i < choiceLocations.Length; i++)
        {
            if (!choiceLocations[i].transform.GetChild(0).gameObject.activeSelf)
            {
                inactiveCount++;
            }
        }
    ...
    // Fisher-Yates shuffle to shuffle the list
